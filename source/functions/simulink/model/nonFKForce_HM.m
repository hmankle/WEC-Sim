    ...
    for kk=1:length(direction)
        ...
        for i=1:length(AH)
            if deepWater == 0 && wDepth <= 0.5*pi/k(i)
                z=(center(:,3)-elv*elvCorr).*wDepth./(wDepth+elv*elvCorr);
                %f_tmp_rp = rho.*g.*sqrt(spread(kk).*AH(i)*dw(i)).*cos(k(i).*X-w(i)*t-phaseRand(i,kk)); % random-phase method
                f_tmp_ra = rho.*g.*(spread(kk).*AH(i)).*cos(k(i).*X-w(i)*t-phaseRand(i,kk)); %random-amplitude method
                fk(:,1) = fk(:,1) + f_tmp_ra.*(cosh(k(i).*(z+wDepth))./cosh(k(i).*wDepth));
            else
                z=(center(:,3)-elv*elvCorr);
                %f_tmp_rp = rho.*g.*sqrt(spread(kk).*AH(i)*dw(i)).*cos(k(i).*X-w(i)*t-phaseRand(i));
                f_tmp_ra = rho.*g.*(spread(kk).*AH(i)).*cos(k(i).*X-w(i)*t-phaseRand(i));
                fk(:,1) = fk(:,1) + f_tmp_ra.*exp(k(i).*z);
            end
        end
        f=fk+f;
    end
